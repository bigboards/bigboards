...
while ! [[ $MASTER_ADDRESS =~ $re ]] ; do
    echo "invalid input!"
    read -r -p "Enter the internal ip addresses of the nodes participating in the cluster. Separate multiple entries with spaces" MASTER_ADDRESS
done
...
read -r -p "Enter the cluster netmask" NETMASK
re='^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$'
while ! [[ $NETMASK =~ $re ]] ; do
    echo "invalid input!"
    read -r -p "Enter the cluster netmask" NETMASK
...
[host-coordinators]
$MASTER_ADDRESS   ansible_ssh_user=bb   bb_node_sequence=1  bb_node_address=$MASTER_ADDRESS
...
for i in $NODE_ADDRESSES; do
    echo "${i}     ansible_ssh_user=bb     bb_node_sequence=${SEQ}  bb_node_address=$i" >> /tmp/ansible_hosts
    let SEQ=SEQ+1
done
